...
function buildStrategyFingerprint(strategy) {
  if (strategy?.fingerprint?.features) {
    return strategy.fingerprint;
  }

  if (strategy?.triggerFeatures || strategy?.flowFeatures) {
    return {
      timeframe:
        strategy.mainSourceTimeframe ||
        strategy.sourceTimeframes?.[0] ||
        strategy.savedTimeframes?.[0] ||
        null,
      direction: strategy.direction,
      features: strategy.triggerFeatures || {},
      flow: strategy.flowFeatures || {},
...
function compareFeatureSet(current, fingerprint, direction, weights) {
  ...
  if (!Object.keys(cf).length || !Object.keys(ff).length) {
...
function matchStrategiesForPair(symbol, currentTfMap, strategies, weights) {
  ...
  for (const strategy of strategies) {
    ...
    const fingerprint = buildStrategyFingerprint(strategy);
    if (!fingerprint?.features || !direction) continue;

    const sourceTf = fingerprint.timeframe || strategy.sourceTimeframes?.[0] || '15m';
    ...
    for (const timeframe of eligibleTfs) {
      ...
      const result = compareFeatureSet(current, fingerprint, direction, weights || {});
      matches.push({
        pair: symbol,
        direction,
